...
def teacher_tab_take_attendance():
    ...
    with col2:
        if st.button('Add Phtos', type="primary", icon=":material/photo_prints:", width='stretch'):
            add_photos_dialog()
    ...
    with c2:
        ...
        if st.button("Run Face Analysis", width='stretch', type='secondary', icon=':material/analytics:', disabled=not st.session_state.attendance_images):
...
def teacher_tab_manage_subjects():
    ...
    # Listing all the created subjects
    subjects = get_teacher_subject(teacher_id)
    if subjects:
        for sub in subjects:
            stats = [
